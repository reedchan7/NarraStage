...
export default router.post(
  ...
  async (req, res) => {
    ...
    const storyboardIds = items
      .filter((item: any) => item.sources == "storyboard")
      .map((item: any) => item.id);
    const totalFilePaths = [];
    if (storyboardIds.length) {
      const storyBoardPaths = await u
        .db("o_storyboard")
        .whereIn("id", storyboardIds)
        .select("id", "filePath");
      totalFilePaths.push(
        ...storyBoardPaths.map((i) => ({ id: i.id, filePath: i.filePath, sources: "storyboard" })),
      );
    }
    const assetsIds = items
      .filter((item: any) => item.sources == "assets")
      .map((item: any) => item.id);
    if (assetsIds.length) {
      ...
      totalFilePaths.push(
        ...assetsPaths.map((i) => ({ id: i.id, filePath: i.filePath, sources: "assets" })),
      );
    }

    await Promise.all(
      totalFilePaths.map(async (item: { id: string; filePath: string; sources: string }) => {
        result[`${item.id}:${item.sources}`] = item.filePath
          ? await u.oss.getSmallImageUrl(item.filePath)
          : "";
      }),
    );
